...
#define kFCDwonloadMaxTaskCount 2

static FCDownloadManager *mgr = nil;

@implementation FCDownloadManager {
...
- (instancetype)init {
    if(self = [super init]) {
        ...
        //注册系统内存不足的通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(systemSpaceInsufficient:) name:FCInsufficientSystemSpaceNotification object:nil];
        //注册程序下载完成的通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downloadTaskDidFinishDownloading:) name:FCDownloadTaskDidFinishNotification object:nil];
        //注册程序即将失去焦点的通知
...
- (void)systemSpaceInsufficient:(NSNotification *)sender {
    ...
    [[FCDownloadManager shredManager] cancelDownloadTask:urlString];
}
...
- (void)downloadTaskWillBeTerminate:(NSNotification *)sender {
    [[FCDownloadManager shredManager] cancelAllTasks];
}
...
- (void)downloadTaskDidFinishDownloading:(NSNotification *)sender {
    ...
    if(_taskDict.count < kFCDwonloadMaxTaskCount) {
        if(_queue.count > 0) {
            @synchronized(_queue) {
...
+ (instancetype)shredManager {
    ...
    dispatch_once(&onceToken, ^{
        mgr = [[FCDownloadManager alloc]init];
    });
    return mgr;
}

- (void)downloadUrl:(NSString *)urlString toPath:(NSString *)destinationPath process:(FCProcessHandle)process completion:(FCDownloadCompletionHandle)completion failure:(FCFailureHandle)failure {
    ...
    FCDownloader *downloader = [FCDownloader downloader];
    @synchronized (self) {
...
- (void)cancelDownloadTask:(NSString *)url {
    FCDownloader *downloader = [_taskDict objectForKey:url];
    [downloader cancel];
    @synchronized (self) {
...
- (float)lastProgress:(NSString *)url {
    return [FCDownloader lastProgress:url];
}

- (NSString *)filesSize:(NSString *)url {
    return [FCDownloader filesSize:url];
}
