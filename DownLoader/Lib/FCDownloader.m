...
#import "FCDownloader.h"
#import "FCTool.h"
#import <UIKit/UIKit.h>

@implementation FCDownloader{
...
+ (NSString *)filesSize:(NSString *)url {
    ...
    NSString *currentSize = [FCTool convertSize:currentLength];
    NSString *totalSize = [FCTool convertSize:totalLength];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    if(freeSpace<1024*1024*20) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:FCInsufficientSystemSpaceNotification object:nil userInfo:@{@"urlString":_url_string}];
        return;
    }
    ...
    NSString *sizeString = [FCDownloader filesSize:_url_string];
    //发送进度改变的通知(一般情况下不需要用到，只有在触发下载与显示下载进度在不同界面的时候才会用到)
    NSDictionary *userInfo = @{@"url" : _url_string,
                               @"progress" : @(progress),
                               @"sizeString" : sizeString};
    [[NSNotificationCenter defaultCenter] postNotificationName:FCProgressDidChangeNotificaiton object:nil userInfo:userInfo];
    //计算网速
    NSString *speedString = @"0.00Kb/s";
    NSString *growString = [FCTool convertSize:_growth*(1.0/0.1)];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    [[NSNotificationCenter defaultCenter] postNotificationName:FCDownloadTaskDidFinishNotification object:nil userInfo:@{@"urlString":_url_string}];
    if(_completion) {
        _completion();
    }
}
